...
// Fetch AdminID using userID from the session
$query = "SELECT AdminID FROM administrator WHERE userID = ?";
...
// Fetch administrator data from the database using AdminID
$query = "SELECT AdminName, AdminPhoneNum, AdminEmail FROM administrator WHERE AdminID = ?";
$stmt = $conn->prepare($query);
if ($stmt === false) {
    die('Error preparing statement: ' . htmlspecialchars($conn->error));
}
...
<body>
    ...
    <form action="AdminProfileUpdate.php" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="AdminName" name="AdminName" value="<?php echo htmlspecialchars($AdminName); ?>">
        </div>
        <div class="mb-3">
